...
void kernel_main(uint32_t multiboot_magic, uintptr_t multiboot_info)
{
    ...
    if (multiboot2_read_memory_map(multiboot_magic, multiboot_info, &memory_map) == 0) {
        multiboot2_log_memory_map(&memory_map);
...
#ifdef AUKOS_EXCEPTION_SELFTEST
    __asm__ volatile ("div %0" : : "r"(0) : "rax", "rdx");
#endif

    for (;;)
    {
        __asm__ volatile ("hlt");//low power idle loop
    }
}
