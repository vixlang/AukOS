...
static int color_from_mode(mode_t mode)
{
  ...
  else if (S_ISREG(mode)) color = 256+37;
...
void ls_main(void)
{
  ...
  if (TT.color) {
    if (!strcmp(TT.color, "never")) toys.optflags &= ~FLAG_color;
    else if (!strcmp(TT.color, "always")) toys.optflags |= FLAG_color;
    else if (!strcmp(TT.color, "auto")) {
      if (stdout_is_tty) toys.optflags |= FLAG_color;
      else toys.optflags &= ~FLAG_color;
    }
  } else if (stdout_is_tty) toys.optflags |= FLAG_color;
