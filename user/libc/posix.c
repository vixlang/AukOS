...
int close(int fd)
{
    return (int)syscall_result(aukos_syscall1(AUKOS_SYS_CLOSE, (uint64_t)fd));
}
...
int open(const char *path, int flags, ...)
{
    ...
    return (int)syscall_result(aukos_syscall3(AUKOS_SYS_OPEN, (uint64_t)path, (uint64_t)flags, (uint64_t)mode));
}
...
{
    return (int)syscall_result(aukos_syscall2(AUKOS_SYS_DUP2, (uint64_t)oldfd, (uint64_t)newfd));
}

int dup(int oldfd)
{
    return (int)syscall_result(aukos_syscall1(AUKOS_SYS_DUP, (uint64_t)oldfd));
}
