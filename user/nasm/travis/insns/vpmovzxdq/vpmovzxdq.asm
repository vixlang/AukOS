default rel
	vpmovzxdq xmm5, qword [0xac4]
	vpmovzxdq xmm0, xmm7
	vpmovzxdq ymm6, xmm7
	vpmovzxdq ymm5, xmm4
	vpmovzxdq xmm2, xmm1
	vpmovzxdq xmm5, qword [0x665]
	vpmovzxdq ymm5, xmm7
	vpmovzxdq ymm4, oword [0xe48]
	vpmovzxdq xmm14, xmm13
	vpmovzxdq ymm12, xmm9
	vpmovzxdq xmm13, xmm15
	vpmovzxdq ymm12, xmm9
	vpmovzxdq xmm17, xmm19
	vpmovzxdq ymm27, xmm28
	vpmovzxdq xmm29, xmm27
	vpmovzxdq ymm18, xmm27
	vpmovzxdq xmm1{k6}, xmm6
	vpmovzxdq ymm1{k2}, oword [0x4cf]
	vpmovzxdq zmm2{k6}, ymm6
	vpmovzxdq xmm1{k7}{z}, xmm3
	vpmovzxdq ymm5{k3}{z}, xmm3
	vpmovzxdq zmm6{k4}{z}, yword [0x596]
	vpmovzxdq xmm4, qword [eax+1]
	vpmovzxdq xmm1, qword [eax+64]
	vpmovzxdq ymm2, oword [eax+1]
	vpmovzxdq ymm1, oword [eax+64]
	vpmovzxdq xmm3, qword [eax+1]
	vpmovzxdq xmm2, qword [eax+64]
	vpmovzxdq ymm0, oword [eax+1]
	vpmovzxdq ymm4, oword [eax+64]
	vpmovzxdq zmm5, yword [eax+1]
	vpmovzxdq zmm4, yword [eax+64]
	vpmovzxdq xmm5, [0xd83]
	vpmovzxdq ymm1, [0xeb3]
	vpmovzxdq xmm1, [0xade]
	vpmovzxdq ymm0, [0x466]
	vpmovzxdq zmm1, [0xe0b]
