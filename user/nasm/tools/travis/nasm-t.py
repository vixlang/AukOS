...
#
# Compress data for a reference file.
#
# If "delta" is given (an int 1..256, the byte distance for the xz
# DELTA filter, as tuned by hand for a specific golden file's fixed
# record stride -- see the "compress" key in the test's .json target
# entry), use exactly that filter chain: this is deterministic and
# fast, and guarantees "update" reproduces the same, already-verified
# small size instead of silently drifting (the previous
# always-try-dist=256 heuristic could be far from optimal: e.g. the
# right distance for mostsecs/manysecs/moresecs turned out to be 192,
# not 256, a ~4x size difference).
#
# If "delta" is None (no "compress" key in the .json yet -- e.g. a
# newly-created large golden), fall back to trying a plain preset vs.
# a dist=256 delta guess and keeping whichever is smaller, same as
# before. Whoever adds a new oversized golden should hand-tune the
# distance (see tools/travis/README.md) and record it in the .json so
# future updates don't need to re-guess.
def compress_ref_data(data, delta = None):
    if delta is not None:
        if delta == 0:
            return lzma.compress(data, preset = 9 | lzma.PRESET_EXTREME)
        filters = [
            {'id': lzma.FILTER_DELTA, 'dist': delta},
...
# compressed; otherwise write plaintext as before.  "delta" is passed
# through to compress_ref_data() -- see the "compress"/"delta" key on
# the target entry in the .json descriptor.
def write_ref_file(path, data, delta = None):
    xz_path = path + '.xz'
    if os.path.exists(xz_path) and not os.path.exists(path):
        with open(xz_path, "wb") as f:
            f.write(compress_ref_data(data, delta))
    else:
...
def test_update(desc):
    ...
    for t in desc['target']:
        delta = t.get('compress', {}).get('delta')
        if 'output' in t:
            output = desc['_base-dir'] + os.sep + t['output']
            match = desc['_base-dir'] + os.sep + t['match']
            ...
            with open(output, "rb") as f:
                data = f.read()
            os.remove(output)
            write_ref_file(match, data, delta)
        if 'stdout' in t:
            match = desc['_base-dir'] + os.sep + t['stdout']
            print("\tMoving %s to %s" % ('stdout', match))
            write_ref_file(match, stdout.encode("utf-8"), delta)
        if 'stderr' in t:
            match = desc['_base-dir'] + os.sep + t['stderr']
            print("\tMoving %s to %s" % ('stderr', match))
            write_ref_file(match, stderr.encode("utf-8"), delta)
